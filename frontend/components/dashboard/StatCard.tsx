...
export function StatCard({ value, label, description, change, changeDirection = 'neutral', delay = 0 }: StatCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="text-brand-muted text-[11px] font-body uppercase tracking-widest mb-1">{label}</div>
      {description && (
        <div className="text-brand-muted text-[11px] font-body mb-2 leading-relaxed">{description}</div>
      )}
      {change && (
