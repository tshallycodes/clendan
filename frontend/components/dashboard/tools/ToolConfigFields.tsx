...
const WORKER_FIELDS: Record<string, FieldDef[]> = {
  reconciliation: [
    ...
    { key: 'run_timezone', type: 'select', label: 'Timezone', options: ['UTC', 'Europe/London', 'Europe/Berlin', 'Europe/Paris', 'America/New_York', 'America/Chicago', 'America/Denver', 'America/Los_Angeles', 'Asia/Dubai', 'Asia/Singapore', 'Asia/Tokyo', 'Australia/Sydney'], default: 'UTC',
      description: 'Timezone for the scheduled run time. For example, set to Europe/London and run hour to 9 to fire at 9 AM London time. Has no effect for Real-time frequency.' },
    { key: 'run_hour_utc', type: 'number', label: 'Run at hour', unit: '0–23', min: 0, max: 23, default: 2,
      description: 'Hour of the day (in the selected timezone) to fire daily or weekly reconciliation. 0 = midnight, 9 = 9:00 AM. Has no effect for Real-time frequency.' },
  ...
  document_intelligence: [
    { key: 'auto_approve_threshold', type: 'number', label: 'Auto-approve under', penceDisplay: true, default: 50000,
      description: 'Invoices below this amount are approved automatically. Above this, a human must review and sign off before payment.' },
    { key: 'po_match_required', type: 'boolean', label: 'PO match required', default: true,
      description: 'When on, every invoice must be matched to an approved purchase order before it can be processed. Prevents unapproved spending.' },
    { key: 'po_tolerance_pct', type: 'number', label: 'PO tolerance', unit: '0–0.10', step: 0.01, min: 0, max: 0.1, default: 0.03,
      description: 'How much the invoice amount can exceed the matched PO value and still be accepted. 0.03 = up to 3% over the PO is fine.' },
    { key: 'duplicate_window_days', type: 'number', label: 'Duplicate window', unit: 'days', default: 90,
      description: 'Look back this many days for a duplicate invoice from the same vendor. Prevents paying the same invoice twice.' },
    { key: 'max_invoice_age_days', type: 'number', label: 'Max invoice age', unit: 'days', default: 180,
      description: 'Reject any invoice dated more than this many days ago. Prevents payment of stale or backdated invoices.' },
    { key: 'new_vendor_flag_enabled', type: 'boolean', label: 'Flag new vendors', default: true,
      description: 'Flag invoices from vendors not previously seen in your system. Helps catch fraudulent or unexpected suppliers before payment.' },
    { key: 'approval_tier_1_limit', type: 'number', label: 'Approval tier 1 limit', penceDisplay: true, default: 100000,
      description: 'Invoices up to this amount are routed to Tier 1 approvers (e.g. team leads or budget holders) for review.' },
    { key: 'approval_tier_2_limit', type: 'number', label: 'Approval tier 2 limit', penceDisplay: true, default: 1000000,
      description: 'Invoices between Tier 1 and this limit go to Tier 2 (e.g. finance managers). Above this goes to senior leadership.' },
    { key: 'receipt_required_above', type: 'number', label: 'Receipt required above', penceDisplay: true, default: 2500,
      description: 'Expenses above this amount must have a receipt or supporting document attached before they can be approved.' },
    { key: 'submission_deadline_days', type: 'number', label: 'Submission deadline', unit: 'days', default: 30,
      description: 'Employees must submit expense claims within this many days of the purchase. Claims submitted late are flagged.' },
    { key: 'ocr_confidence_min', type: 'number', label: 'OCR min confidence', unit: '0–1', step: 0.01, min: 0, max: 1, default: 0.82,
      description: 'Minimum confidence for OCR-extracted data to be accepted automatically. Below this, the document is flagged for manual data entry.' },
    { key: 'duplicate_receipt_window_days', type: 'number', label: 'Duplicate receipt window', unit: 'days', default: 365,
      description: 'Look back this many days for a duplicate receipt from the same person or vendor. Catches repeat submissions.' },
