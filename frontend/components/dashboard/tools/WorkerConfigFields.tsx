...
const WORKER_FIELDS: Record<string, FieldDef[]> = {
  ...
  receipt_processing: [
    ...
    { key: 'max_receipt_age_days',             type: 'number',  label: 'Max receipt age',             unit: 'days', default: 90 },
    ...
    { key: 'auto_approve_below',               type: 'number',  label: 'Auto-approve below',          penceDisplay: true, default: 1000 },
    { key: 'missing_receipt_grace_period_days',type: 'number',  label: 'Missing receipt grace period',unit: 'days', default: 7 },
  ],
  reconciliation: [
...
export function WorkerConfigFields({ workerType, config, onChange }: WorkerConfigFieldsProps) {
  ...
  return (
    <div className="space-y-3 border-t border-brand-border pt-3">
      <p className={labelClass}>Worker Settings</p>
      {fields.map((field) => {
