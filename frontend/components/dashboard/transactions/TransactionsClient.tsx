  ...
  async function loadMore() {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/transactions?limit=${PAGE_SIZE}&offset=${offset}`,
        { headers: { Authorization: `Bearer ${token}` } },
      )
      if (!res.ok) return
      const json = await res.json()
    ...
    <motion.div variants={pageVariants} initial="hidden" animate="show" className="p-6 space-y-6">
      ...
      <motion.div variants={sectionVariants} className="flex items-start justify-between gap-4 flex-wrap">
        ...
        <input
          ...
          className="bg-brand-bg border border-brand-border focus:border-[#00C853] text-brand-text placeholder:text-brand-muted rounded-sm px-3 py-1.5 text-xs font-body w-64 outline-none transition-colors"
        />
      </motion.div>
