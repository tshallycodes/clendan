...
const DECISION_STYLES: Record<string, { label: string; className: string }> = {
  ...
  routed:             { label: 'Queued',              className: 'bg-[#111118] text-[#a0b8a0] border border-[#1a2a1a]' },
  queued:             { label: 'Queued',              className: 'bg-[#111118] text-[#a0b8a0] border border-[#1a2a1a]' },
}
...
export function WorkerTestResult({ result, onDismiss }: Props) {
  useEffect(() => {
  ...
  if (result.error) {
    return (
      <div className="flex items-center justify-between mt-3 px-3 py-2 bg-[rgba(255,77,109,0.06)] border border-[rgba(255,77,109,0.2)] rounded-sm">
        ...
        <button type="button" onClick={onDismiss} className="text-[#ff4d6d] text-[10px] font-mono ml-3 hover:opacity-70 transition-opacity">×</button>
      </div>
    )
  }

  const style = DECISION_STYLES[result.decision] ?? { label: result.decision, className: 'bg-[#111118] text-[#a0b8a0] border border-[#1a2a1a]' }
  const isQueued = result.decision === 'routed' || result.decision === 'queued'

  return (
    <div className="flex items-center justify-between mt-3 px-3 py-2 bg-[#0a0a0f] border border-[#1a2a1a] rounded-sm">
      ...
      <button type="button" onClick={onDismiss} className="text-[#4a6a4a] text-[10px] font-mono ml-3 hover:text-[#a0b8a0] transition-colors">×</button>
    </div>
  )
}
