  ...
  return (
    <div className="flex items-end gap-2 p-3 border-t border-brand-border">
      <textarea
        ...
        rows={1}
        disabled={disabled}
        className="flex-1 resize-none bg-brand-bg border border-brand-border focus:border-brand-green focus:outline-none rounded-sm px-3 py-2 text-xs font-body text-brand-text placeholder:text-brand-muted transition-colors leading-5 min-h-[36px] disabled:opacity-50 overflow-y-hidden"
      />
      <button
        onClick={submit}
        disabled={!canSend}
