...
export function StatusLabel({ status }: { status: IntegrationStatus }) {
  if (status === 'connected') {
    return <span className="text-[11px] font-body text-[#00C853] tracking-wider">CONNECTED</span>
  }
  if (status === 'syncing') {
    return <span className="text-[11px] font-body text-[#00a8cc] tracking-wider">SYNCING</span>
  }
  if (status === 'connecting') {
    return <span className="text-[11px] font-body text-[#00a8cc] tracking-wider">CONNECTING</span>
  }
  if (status === 'error') {
    return <span className="text-[11px] font-body text-[#ff4d6d] tracking-wider">ERROR</span>
  }
  return null
}
