...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className="text-[11px] font-body uppercase tracking-wider px-2 py-0.5 rounded-sm"
    ...
    <div className="bg-brand-bg border border-brand-border rounded-sm overflow-hidden" style={{ borderLeft: '3px solid #00C853' }}>
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-brand-border">
        <div>
          ...
          <p className="text-[11px] font-body text-brand-muted mt-0.5">Last sync: {lastSyncDisplay}</p>
        </div>
          ...
          {conn.accounts.map((a) => (
            <div key={a.id} className="flex items-center justify-between gap-4 px-4 py-2.5">
              <div className="min-w-0">
                ...
                <p className="text-[11px] font-body text-brand-muted uppercase mt-0.5">{a.subtype || a.type}</p>
              </div>
      ...
      {conn.recent_transactions.length > 0 && (
        <div className="border-b border-brand-border">
          <p className="text-[11px] font-body uppercase tracking-widest text-brand-muted px-4 pt-3 pb-2">Recent</p>
          <div className="divide-y divide-brand-border">
            {conn.recent_transactions.slice(0, 3).map((t) => (
              <div key={t.id} className="flex items-center justify-between gap-4 px-4 py-2">
                <p className="text-[11px] font-body text-brand-secondary truncate">{t.merchant_name || t.description}</p>
                <div className="text-right shrink-0">
                  <p className="text-[11px] font-body text-brand-text">{convert(t.amount_minor, t.currency)}</p>
      ...
      <div className="px-4 py-3 flex gap-2">
        <button
          ...
          className="flex-1 py-1.5 text-[11px] font-body text-brand-text bg-transparent border border-brand-border rounded-sm hover:bg-brand-elevated transition-colors disabled:opacity-60"
        >
        ...
        <button
          ...
          className="flex-1 py-1.5 text-[11px] font-body text-brand-text bg-transparent border border-brand-border rounded-sm hover:bg-brand-elevated transition-colors"
        >
          Sync Log
        </button>
        {confirmDisconnect ? (
          <>
            <button
              ...
              className="flex-1 py-1.5 text-[11px] font-body text-[#ff4d6d] bg-[rgba(255,77,109,0.1)] border border-[#ff4d6d] rounded-sm hover:bg-[rgba(255,77,109,0.15)] transition-colors disabled:opacity-60"
            >
              {disconnecting ? 'Disconnecting...' : 'Confirm'}
            </button>
            <button
              onClick={() => setConfirmDisconnect(false)}
              className="px-3 py-1.5 text-[11px] font-body text-brand-muted bg-transparent border border-brand-border rounded-sm hover:bg-brand-elevated transition-colors"
            >
              Cancel
            </button>
          </>
          ...
          <button
            ...
            className="flex-1 py-1.5 text-[11px] font-body text-[#ff4d6d] bg-[rgba(255,77,109,0.1)] border border-[#ff4d6d]/30 rounded-sm hover:bg-[rgba(255,77,109,0.15)] transition-colors"
          >
            Disconnect
          </button>
        )}
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  {logoError || !bank.domain ? (
                    <span className="text-[11px] font-body font-bold text-white leading-none">
                      {bank.abbr.slice(0, 2)}
                    </span>
                  ) : (
                    <Image
              ...
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-16 gap-5 text-center">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] font-body text-brand-muted mt-1.5 max-w-[220px] mx-auto leading-relaxed">
