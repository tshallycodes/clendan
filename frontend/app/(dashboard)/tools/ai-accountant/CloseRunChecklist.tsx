  ...
  return (
    <div className="bg-brand-surface border border-brand-border rounded-sm overflow-hidden">
      <div className="px-4 py-3 border-b border-brand-border">
        <p className="text-[11px] font-body uppercase tracking-widest text-brand-muted">Checklist</p>
      </div>
            ...
            <div key={task.task_key} className="px-4 py-3 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {task.notes && (
                    <p className="text-[11px] font-body text-brand-muted mt-0.5 truncate">{task.notes}</p>
                  )}
                </div>
              </div>
              <div className="flex items-center gap-2 shrink-0">
                <span className={`text-[11px] font-body px-2 py-0.5 rounded-sm ${STATUS_BADGE_CLS[task.status] ?? ''}`}>
                  {task.status}
                </span>
                {canComplete && (
                  <button
