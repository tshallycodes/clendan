...
function statusBadge(item: ReconciliationItem) {
  ...
  if (item.status === 'matched')
    return (
      <span className="px-2 py-0.5 rounded-sm text-[10px] font-body bg-[rgba(0,200,83,0.08)] text-[#00C853] border border-[rgba(0,200,83,0.2)]">
        matched
      </span>
    )
  if (aiAction === 'flag')
    return (
      <span className="px-2 py-0.5 rounded-sm text-[10px] font-body bg-[rgba(255,77,109,0.08)] text-[#ff4d6d] border border-[rgba(255,77,109,0.2)]">
        flagged
      </span>
    )
  if (aiAction === 'review')
    return (
      <span className="px-2 py-0.5 rounded-sm text-[10px] font-body bg-[rgba(0,168,204,0.08)] text-[#00a8cc] border border-[rgba(0,168,204,0.2)]">
        review
      </span>
    )
  if (aiAction === 'ok')
  ...
  return (
    <span className="px-2 py-0.5 rounded-sm text-[10px] font-body bg-brand-elevated text-brand-muted border border-brand-border">
      {item.status}
    </span>
  )
}
      ...
      <div className="flex flex-col gap-2 px-4 py-3 border-b border-brand-border">
        <div className="flex items-center justify-between gap-2">
          <div className="flex gap-1 flex-wrap">
            {tabs.map((tab) => (
              <button
                ...
                className={`px-3 py-1 text-[10px] font-body rounded-sm transition-colors ${
                  activeAccount === tab
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-[10px] font-body text-brand-secondary border border-brand-border px-3 py-1.5 rounded-sm hover:text-brand-text hover:border-brand-text transition-colors shrink-0"
          >
        ...
        <div className="flex gap-1">
          {AI_FILTERS.map((f) => (
            <button
              ...
              className={`px-3 py-1 text-[10px] font-body rounded-sm border transition-colors ${
                activeFilter === f.key
                  ? f.key === 'flag'
            ...
            <thead>
              <tr className="border-b border-brand-border">
                {['Date', 'Account', 'Description', 'Amount', 'Status', 'Invoice #', 'Vendor', 'Source', 'Agent Reasoning'].map(
                  (h) => (
                    <th key={h} className="px-3 py-2 text-left text-[10px] uppercase tracking-widest text-brand-muted font-body whitespace-nowrap">
                      {h}
                    </th>
                  ),
                )}
                  ...
                  <Fragment key={item.id}>
                    ...
                    >
                      ...
                      <td className="px-3 py-2">
                        {item.matched_source
                          ? <span className="px-2 py-0.5 rounded-sm text-[10px] font-body bg-brand-elevated text-brand-secondary border border-brand-border capitalize">{item.matched_source}</span>
                    ...
                    {isExpanded && item.reasoning && (
                      <tr className="border-b border-brand-border bg-brand-elevated">
                        <td colSpan={9} className="px-4 py-3">
                          <p className={`text-[11px] font-body leading-relaxed ${item.ai_action === 'flag' ? 'text-[#ff4d6d]' : item.ai_action === 'review' ? 'text-[#00a8cc]' : 'text-brand-secondary'}`}>
                            {item.reasoning}
                          </p>
                          {item.ai_severity && (
                            <p className="text-[10px] font-body text-brand-muted mt-1">Severity: {item.ai_severity}</p>
                          )}
                        </td>
                      </tr>
                    )}
