    ...
    <div className="space-y-2">
      <div className="relative">
        ...
        <input
          ...
          className="w-full bg-brand-bg border border-brand-border focus:border-[#00C853] text-brand-text placeholder:text-brand-muted rounded-sm pl-8 pr-3 py-2 text-xs font-mono outline-none transition-colors"
        />
      </div>
      <div className="border border-brand-border rounded-sm overflow-hidden max-h-60 overflow-y-auto">
        {filtered.length === 0 ? (
          <div className="px-3 py-3 text-xs font-mono text-brand-muted">No runs match your search.</div>
        ) : (
          filtered.map((run, i) => (
            <button
              key={run.id}
              ...
              className={`w-full text-left px-3 py-2.5 text-xs font-mono transition-colors flex items-center justify-between gap-4 ${
