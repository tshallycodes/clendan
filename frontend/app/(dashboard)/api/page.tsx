...
with open("assessments.csv", "w", newline="") as f:
            ...
            <table className="w-full text-xs font-body border border-brand-border rounded-sm">
              ...
              <tbody className="divide-y divide-brand-border">
                {[
                  ...
                  ['policy.match_amount_tolerance_minor', 'integer',   '150',    'Allowed absolute difference (minor units) when matching amounts'],
                  ['policy.match_date_window_days',       'integer',   '5',      'Days either side of due date to still count as a match'],
                  ['policy.reconciliation_frequency',     'string',    '"daily"','How often the tool runs: daily, weekly, or real-time'],
                  ['policy.stale_open_item_days',         'integer',   '90',     'Days before an unmatched item is flagged as stale'],
                  ['policy.unmatched_alert_days',         'integer',   '5',      'Days an item stays unmatched before it is escalated for review'],
  ...
  "payload": {
    ...
    "policy": {
      "unmatched_pct_threshold":      0.2,
      "match_amount_tolerance_minor": 150,
