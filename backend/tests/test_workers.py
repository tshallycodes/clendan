...
class TestListWorkers:
    ...
    async def test_returns_tenant_scoped_list(self):
        ...
        result = await list_workers(current_user=FAKE_USER, db=db)
...
class TestGetWorker:
    ...
    async def test_returns_worker_for_correct_tenant(self):
        ...
        result = await get_worker(worker_id="worker_001", current_user=FAKE_USER, db=db)
    ...
    async def test_returns_404_for_nonexistent_worker(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await get_worker(worker_id="does_not_exist", current_user=FAKE_USER, db=db)
...
class TestDeactivateWorker:
    ...
    async def test_sets_status_inactive_does_not_delete(self):
        from app.api.v1.workers import pause_worker
        ...
        result = await pause_worker(worker_id="worker_001", current_user=FAKE_USER, db=db)
    ...
    async def test_does_not_call_db_delete(self):
        ...
        await pause_worker(worker_id="worker_001", current_user=FAKE_USER, db=db)
