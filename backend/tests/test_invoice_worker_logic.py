"""
Unit tests for execute_invoice_worker business logic.
...
class TestExecuteInvoiceWorker:
    ...
    async def test_auto_approved_when_all_rules_pass(self, mock_db, mock_audit):
        ...
        with (
            patch("app.workers.invoice_processing._extract_invoice", AsyncMock(return_value=parsed)),
            patch("app.workers.invoice_processing.get_db", return_value=mock_db),
            ...
            result = await execute_invoice_worker(
                worker_id="w1", tenant_id="t1", execution_id="e1",
    ...
    async def test_blocked_on_duplicate_invoice(self, mock_audit):
        ...
        with (
            patch("app.workers.invoice_processing._extract_invoice", AsyncMock(return_value=parsed)),
            patch("app.workers.invoice_processing.get_db", return_value=db),
            patch("app.workers.invoice_processing.write_audit_log", mock_audit),
        ):
            from app.workers.invoice_processing import execute_invoice_worker
            result = await execute_invoice_worker(
    ...
    async def test_extract_failure_propagates(self, mock_db, mock_audit):
        with (
            patch("app.workers.invoice_processing._extract_invoice",
...
class TestCheckDuplicate:
    ...
    async def test_returns_true_when_duplicate_exists(self):
        ...
        with patch("app.workers.invoice_processing.get_db", return_value=db):
            from app.workers.invoice_processing import _check_duplicate
