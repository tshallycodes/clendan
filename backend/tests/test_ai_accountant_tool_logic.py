"""
Unit tests for AIAccountantTool business logic.
...
from app.tools.ai_accountant import AIAccountantTool, TransactionResult
...
class TestAIAccountantToolRun:
    ...
    async def test_auto_decision_when_high_confidence(self):
        ...
        with (
            patch("app.tools.ai_accountant.get_db", return_value=db),
            patch("app.tools.ai_accountant.get_trace_id", return_value="trace_1"),
            patch("app.tools.ai_accountant.anthropic.AsyncAnthropic") as mock_anthropic,
        ):
            ...
            result = await tool.run(["txn_1"], "tenant_1", "tool_1")
    ...
    async def test_raises_when_no_transactions_found(self):
        ...
        with patch("app.tools.ai_accountant.get_db", return_value=db):
            tool = AIAccountantTool()
            with pytest.raises(ValueError, match="No transactions found"):
                await tool.run(["txn_missing"], "tenant_1", "tool_1")
    ...
    async def test_raises_on_empty_transaction_ids(self):
        ...
        with patch("app.tools.ai_accountant.get_db", return_value=db):
            ...
            with pytest.raises(ValueError, match="transaction_ids cannot be empty"):
                await tool.run([], "tenant_1", "tool_1")
    ...
    async def test_audit_always_written(self):
        ...
        with (
            ...
            tool = AIAccountantTool()
            await tool.run(["txn_1"], "tenant_1", "tool_1")
    ...
    async def test_policy_config_overrides_thresholds(self):
        ...
        with (
            ...
            tool = AIAccountantTool()
            result = await tool.run(["txn_1"], "tenant_1", "tool_1", policy_config=policy)

        assert result.decision == "auto"

    @pytest.mark.asyncio
    async def test_retries_on_transient_api_error(self):
        import app.tools.ai_accountant as acct_mod
        db = _make_db()
        ...
        with (
            ...
            patch("app.tools.ai_accountant.get_trace_id", return_value="trace_1"),
            patch("app.tools.ai_accountant.anthropic.AsyncAnthropic") as mock_anthropic,
            patch("app.tools.ai_accountant.asyncio.sleep", AsyncMock()),
        ):
            mock_client = AsyncMock()
            mock_anthropic.return_value = mock_client
