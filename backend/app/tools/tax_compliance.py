...
def _parse_policy(config_json: dict) -> _ToolPolicy:
    ...
    return _ToolPolicy(
        vat_alert_threshold_cents=raw.get("vat_alert_threshold_cents", 1_000_000),
        missing_tax_flag_threshold_cents=raw.get("missing_tax_flag_threshold_cents", 10_000),
    )
...
def _build_prompt(
    ...
    data = {
        ...
        "input_vat_reclaimable_cents": input_vat,
        "net_vat_liability_cents": net_vat,
        "missing_tax_invoices": missing_invoice,
        "missing_tax_bills": missing_bill,
        "missing_tax_expenses": missing_expense,
        "available_tax_rates": tax_rates,
        "vat_alert_threshold_cents": policy.vat_alert_threshold_cents,
    }
    return (
...
async def _execute(tenant_id: str, tool_id: str, execution_id: str, payload: dict) -> dict:
    ...
    lookback = datetime.now(UTC) - timedelta(days=_LOOKBACK_DAYS)
    ...
    invoices, bills, expenses, tax_rates = await asyncio.gather(
        ...
        db.accountingbill.find_many(
            where={"tenant_id": tenant_id, "issue_date": {"gte": lookback}}
        ),
        db.accountingexpense.find_many(
            where={"tenant_id": tenant_id, "issue_date": {"gte": lookback}}
        ),
        db.accountingtaxrate.find_many(
    ...
    if not invoices and not bills and not expenses:
        await write_audit_log(
            tenant_id=tenant_id,
            actor=_ACTOR,
            action="tax_compliance:no_action",
        ...
        return {
            "decision": "no_action",
            "confidence": 1.0,
            "reasoning": "No transactions found in the last 90 days.",
            "actions_taken": [],
            "output_data": {},
        }
    ...
    input_vat = sum(
        (getattr(b, "tax_cents", None) or 0) for b in bills
    ) + sum(
        (getattr(e, "tax_cents", None) or 0) for e in expenses
    )
    net_vat = vat_collected - input_vat
    ...
    claude_assessment = await _call_claude(
        vat_collected, input_vat, net_vat,
        missing_invoice, missing_bill, missing_expense,
        tax_rate_list, policy, settings_obj,
    )

    threshold_breached = net_vat > policy.vat_alert_threshold_cents
    total_missing = len(missing_invoice) + len(missing_bill) + len(missing_expense)
    overall = "vat_alert" if threshold_breached else ("compliance_review" if total_missing > 0 else "auto_approved")
    confidence = 0.85 if overall != "auto_approved" else 0.92

    reasoning_trace = {
        "overall_decision": overall,
        "vat_collected_cents": vat_collected,
        "input_vat_cents": input_vat,
        "net_vat_cents": net_vat,
        "threshold_breached": threshold_breached,
        "missing_tax_count": {
            "invoices": len(missing_invoice),
            "bills": len(missing_bill),
            "expenses": len(missing_expense),
        },
    ...
    await write_audit_log(
        tenant_id=tenant_id,
        actor=_ACTOR,
        action=f"tax_compliance:{overall}",
    ...
    actions_taken = [
        ...
        f"net VAT liability: {net_vat} cents",
    ]
    if threshold_breached:
        actions_taken.append(
            f"VAT ALERT: net liability {net_vat} exceeds threshold {policy.vat_alert_threshold_cents}"
        )
    if total_missing:
        actions_taken.append(f"flagged {total_missing} transaction(s) with missing tax")
    ...
    try:
        ...
        if ctx.get("job_try", 1) >= 3:
            await push_to_dlq(
                ...
                function_name="run_tax_compliance_job",
                error=str(exc),
            )
        raise
