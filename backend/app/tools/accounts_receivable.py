...
def _build_prompt(invoices: list[_InvoiceRecord]) -> str:
...
async def _call_claude(invoices: list[_InvoiceRecord], settings_obj) -> _ClaudeOutput:
    client = AsyncAnthropic(api_key=settings_obj.anthropic_api_key)
    prompt = _build_prompt(invoices)
...
async def _execute(tenant_id: str, tool_id: str, execution_id: str, payload: dict) -> dict:
    ...
    today = date.today()
    raw = await db.accountinginvoice.find_many(
        where={
            "tenant_id": tenant_id,
    ...
    claude_out = await _call_claude(invoices, settings_obj)
    ...
    confidence = 0.90 if has_high else (0.80 if has_medium else 0.95)
