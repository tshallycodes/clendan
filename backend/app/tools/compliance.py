"""
Compliance Tool — sub-agent tool. Called by Financial Orchestrator as a tool.
...
from app.tools.compliance_policy import _ToolConfig, _parse_config

logger = get_logger(__name__)

_ACTOR = "tool:compliance:v1"
...
def _active_frameworks(config: _ToolConfig, requested: list[str]) -> list[str]:
    """Intersect requested frameworks with those enabled in tool config."""
...
async def _execute_compliance(
    ...
    if not active:
        raise ValueError(
            ...
            f"Tool config enables: {config.frameworks}"
        )
