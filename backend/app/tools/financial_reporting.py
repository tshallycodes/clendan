...
def _build_snapshot(
    ...
    cash_inflows_cents = sum(
        p.amount_cents for p in payments if p.payment_type == "receipt" and in_period(p.paid_at)
    )
    cash_outflows_cents = sum(
        p.amount_cents for p in payments if p.payment_type == "payment" and in_period(p.paid_at)
    )
    ...
    return _FinancialSnapshot(
        ...
        outstanding_ar_cents=outstanding_ar_cents,
        cogs_cents=cogs_cents,
        opex_cents=opex_cents,
        outstanding_ap_cents=outstanding_ap_cents,
        ...
        net_assets_cents=net_assets_cents,
...
def _build_prompt(snapshot: _FinancialSnapshot, policy: _ToolPolicy) -> str:
    data = snapshot.model_dump()
    return (
        "You are a CFO-level financial analyst. Analyse the financial snapshot below and return a JSON object "
        "with exactly these fields:\n"
        '  "pl_summary": string (3-4 sentences: revenue, gross profit margin %, net profit, key drivers),\n'
        '  "balance_sheet_summary": string (2-3 sentences: asset position, liabilities, net asset strength),\n'
        '  "cash_flow_summary": string (2-3 sentences: cash inflows vs outflows, net cash movement),\n'
        '  "anomalies": array of strings (each a specific concern — empty array if none),\n'
        '  "has_anomaly": boolean,\n'
        '  "overall_health": "strong"|"stable"|"at_risk",\n'
        '  "recommendations": array of up to 3 action strings\n\n'
        "Flag anomalies when: gross margin < 0, net profit negative, AP > AR by >20%, "
        f"or any figure varies more than {int(policy.anomaly_variance_pct * 100)}% from what would be expected.\n"
        "Return ONLY valid JSON. No markdown, no prose.\n\n"
        f"Financial snapshot:\n{json.dumps(data, indent=2)}"
    )


async def _call_claude(snapshot: _FinancialSnapshot, policy: _ToolPolicy, settings_obj) -> dict:
    client = AsyncAnthropic(api_key=settings_obj.anthropic_api_key)
    prompt = _build_prompt(snapshot, policy)
...
async def _execute(tenant_id: str, tool_id: str, execution_id: str, payload: dict) -> dict:
    ...
    if not invoices and not bills and not expenses:
        await write_audit_log(
            tenant_id=tenant_id,
            actor=_ACTOR,
            action="financial_reporting:no_data",
            reasoning_trace={"reason": "no_accounting_data"},
        ...
        return {
            "decision": "no_action",
            "confidence": 1.0,
            "reasoning": "No accounting data available to generate report.",
            "actions_taken": [],
            "output_data": {},
        }

    snapshot = _build_snapshot(invoices, bills, expenses, payments, bank_accounts, period_start, period_end)
    claude_result = await _call_claude(snapshot, policy, settings_obj)

    has_anomaly: bool = claude_result.get("has_anomaly", False)
    overall_health: str = claude_result.get("overall_health", "stable")

    if has_anomaly or overall_health == "at_risk":
    ...
    await write_audit_log(
        tenant_id=tenant_id,
        actor=_ACTOR,
        action=f"financial_reporting:{overall}",
    ...
    actions_taken = [
        ...
        f"analysed {snapshot.invoice_count} invoices, {snapshot.bill_count} bills, "
...
async def run_financial_reporting_job(
    ctx: dict,
    *,
    execution_id: str,
    tenant_id: str,
    ...
    try:
        ...
        if ctx.get("job_try", 1) >= 3:
            await push_to_dlq(
                ...
                function_name="run_financial_reporting_job",
                error=str(exc),
            )
        raise
